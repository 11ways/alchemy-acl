...
/**
 * Turn the given error into a public error message
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    0.8.7
 * @version  0.8.7
 ...
 * @return   {string}
 */
Proteus.setMethod(function getPublicErrorMessage(err) {

	if (!err) {
		return 'Unknown proteus error';
	}
	...
	if (alchemy.settings.debug) {
		...
		result += ' (proteus server response)';
	}
 ...
 * @version  0.8.4
 *
 * @param    {Acl.PersistentCookie}   document
 */
 ...
 * @version  0.8.4
 *
 * @param    {Acl.PersistentCookie}   cookie
 * @param    {User}                  user
