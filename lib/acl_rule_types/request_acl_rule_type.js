...
alchemy.create('AclRuleType', function RequestAclRuleType (){
	...
	this.allowed = function allowed(rule, user, req) {
		log.acl(this.name + 'has ' + 'allowed'.bold.green + ' user ' + user.username + ' access to url ' + req.originalUrl);
	};
	...
	this.denied = function denied(rule, user, req) {
		log.acl(this.name + ' has ' + 'denied'.bold.red + ' user ' + user.username + ' access to url ' + req.originalUrl);
	};
